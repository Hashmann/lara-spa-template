<template>
  <q-header reveal elevated class="bg-primary text-white" height-hint="98">
    <div class="row no-wrap shadow-1">
      <q-toolbar>
        <q-btn size="md" flat @click.prevent="$emit('toggleDrawer')" round dense icon="menu"/>
...
<!--        $q.dark.toggle()-->
        ...
        <q-btn size="md" flat round dense icon="sms"/>
        <q-btn round push size="md">
          <q-avatar>
          ...
          <q-menu fit anchor="bottom left" transition-show="jump-down" transition-hide="jump-up" self="top left">
            <div class="row no-wrap q-pa-md">
              <div class="column items-center">
                ...
                <router-link :to="{name: 'dashboard'}"><q-btn color="primary" label="Профиль" push size="sm" v-close-popup/></router-link>
                <q-btn @click.prevent="logout" color="primary" label="Выход" push size="sm" v-close-popup/>
              </div>
            </div>
          </q-menu>
        </q-btn>
...
export default {
  name: "AdminBar",
  mounted() {
    console.log("mounted AdminBar")
    // this.user.theme = this.getCurrentUser.theme
  },
  unmounted() {
    console.log("unmounted AdminBar")
  },
  setup() {
    // const $q = useQuasar()
  },
